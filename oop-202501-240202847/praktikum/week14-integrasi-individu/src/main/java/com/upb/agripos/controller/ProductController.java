package com.upb.agripos.controller;

import java.util.List;

import javax.swing.DefaultListModel;
import javax.swing.JOptionPane;
import javax.swing.table.DefaultTableModel;

import com.upb.agripos.model.Cart;
import com.upb.agripos.model.CartItem;
import com.upb.agripos.model.Product;
import com.upb.agripos.service.ProductService;
import com.upb.agripos.view.ProductFormView;

public class ProductController {
    private final ProductFormView view;
    private final ProductService service;
    private final Cart cart = new Cart();

    public ProductController(ProductFormView view, ProductService service) {
        this.view = view;
        this.service = service;
        initEventHandlers();
        loadData();
    }

    private void initEventHandlers() {
        // Event-Driven: Aksi saat tombol diklik
        view.btnAdd.addActionListener(event -> {
            try {
                Product p = new Product(
                    view.txtCode.getText(),
                    view.txtName.getText(),
                    Double.parseDouble(view.txtPrice.getText()),
                    Integer.parseInt(view.txtStock.getText())
                );
                
                service.addProduct(p);
                refreshList();
                clearForm();
                
            } catch (Exception e) {
                showError("Gagal Tambah: " + e.getMessage());
            }
        });

        view.btnUpdate.addActionListener(event -> {
            try {
                Product p = new Product(
                    view.txtCode.getText(),
                    view.txtName.getText(),
                    Double.parseDouble(view.txtPrice.getText()),
                    Integer.parseInt(view.txtStock.getText())
                );
                
                service.updateProduct(p);
                refreshList();
                clearForm();
                
            } catch (Exception e) {
                showError("Gagal Update: " + e.getMessage());
            }
        });

        view.btnDelete.addActionListener(event -> {
            try {
                String code = view.txtCode.getText();
                if (code.isEmpty()) {
                    showError("Pilih produk dari daftar untuk dihapus!");
                    return;
                }
                service.deleteProduct(code);
                refreshList();
                clearForm();
                
            } catch (Exception e) {
                showError("Gagal Hapus: " + e.getMessage());
            }
        });

        view.btnSearch.addActionListener(event -> {
            try {
                String query = view.txtSearch.getText();
                List<Product> results = service.searchProducts(query);
                updateList(results);
                
            } catch (Exception e) {
                showError("Gagal Cari: " + e.getMessage());
            }
        });

        view.btnAddToCart.addActionListener(event -> {
            try {
                String code = view.txtCode.getText();
                int qty = Integer.parseInt(view.txtQuantity.getText());
                Product p = service.findProduct(code);
                if (p != null && qty > 0) {
                    cart.addItem(p, qty);
                    updateCartList();
                    JOptionPane.showMessageDialog(view, "Ditambahkan ke keranjang!");
                } else {
                    showError("Produk tidak ditemukan atau qty invalid!");
                }
            } catch (Exception e) {
                showError("Gagal Tambah ke Keranjang: " + e.getMessage());
            }
        });

        view.btnViewCart.addActionListener(event -> {
            updateCartList();
        });

        view.btnCheckout.addActionListener(event -> {
            if (cart.getItems().isEmpty()) {
                showError("Keranjang kosong!");
                return;
            }
            showCheckoutSummary();
        });

        // List selection listener
        view.listView.getSelectionModel().addListSelectionListener(event -> {
            if (!event.getValueIsAdjusting()) {
                int row = view.listView.getSelectedRow();
                if (row != -1) {
                    try {
                        String code = (String) view.listView.getValueAt(row, 0);
                        Product p = service.findProduct(code);
                        if (p != null) {
                            view.txtCode.setText(p.getCode());
                            view.txtName.setText(p.getName());
                            view.txtPrice.setText(String.valueOf(p.getPrice()));
                            view.txtStock.setText(String.valueOf(p.getStock()));
                        }
                    } catch (Exception e) {
                        showError("Gagal Load Produk: " + e.getMessage());
                    }
                }
            }
        });
    }

    private void loadData() {
        try {
            DefaultListModel<String> model = (DefaultListModel<String>) view.listView.getModel();
            List<Product> products = service.getAllProducts();
            products.forEach(p -> 
                model.addElement(p.getCode() + " - " + p.getName() + " [Rp" + p.getPrice() + "]"));
            updateStatus(products);
        } catch (Exception e) {
            showError("Gagal Load Data: " + e.getMessage());
        }
    }

    private void refreshList() throws Exception {
        DefaultTableModel model = (DefaultTableModel) view.listView.getModel();
        model.setRowCount(0);
        List<Product> products = service.getAllProducts();
        products.forEach(p -> 
            model.addRow(new Object[]{p.getCode(), p.getName(), String.format("Rp %,d", (int)p.getPrice()), p.getStock()}));
        updateStatus(products);
    }

    private void updateList(List<Product> products) {
        DefaultTableModel model = (DefaultTableModel) view.listView.getModel();
        model.setRowCount(0);
        products.forEach(p -> 
            model.addRow(new Object[]{p.getCode(), p.getName(), String.format("Rp %,d", (int)p.getPrice()), p.getStock()}));
        updateStatus(products);
    }

    private void updateStatus(List<Product> products) {
        int total = products.size();
        double totalValue = products.stream().mapToDouble(p -> p.getPrice() * p.getStock()).sum();
        view.lblStatus.setText("Total Produk: " + total + " | Total Nilai: Rp" + totalValue + " | Keranjang: " + cart.getItems().size() + " item(s)");
    }

    private void updateCartList() {
        DefaultListModel<String> model = (DefaultListModel<String>) view.cartList.getModel();
        model.clear();
        cart.getItems().forEach(item -> 
            model.addElement(String.format("%s x%d = %s", item.getProduct().getName(), item.getQuantity(), String.format("Rp %,d", (int)item.getSubtotal()))));
        try {
            updateStatus(service.getAllProducts());
        } catch (Exception e) {
            // ignore
        }
    }

    private void showCheckoutSummary() {
        StringBuilder summary = new StringBuilder("=== RINGKASAN CHECKOUT ===\n\n");
        for (CartItem item : cart.getItems()) {
            summary.append(item.toString()).append("\n");
        }
        summary.append("\nTOTAL: Rp").append(cart.getTotal());
        
        int result = JOptionPane.showConfirmDialog(view, summary.toString(), "Konfirmasi Checkout", JOptionPane.YES_NO_OPTION);
        if (result == JOptionPane.YES_OPTION) {
            cart.clear();
            updateCartList();
            JOptionPane.showMessageDialog(view, "Checkout berhasil!");
        }
    }

    private void clearForm() {
        view.txtCode.setText("");
        view.txtName.setText("");
        view.txtPrice.setText("");
        view.txtStock.setText("");
    }

    private void showError(String msg) {
        JOptionPane.showMessageDialog(view, msg, "Error", JOptionPane.ERROR_MESSAGE);
    }
}